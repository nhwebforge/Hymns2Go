...
# Array of hymnal codes and names
declare -A hymnals=(
  ["GG2013"]="Gather Comprehensive Second Edition"
  ["UMH"]="United Methodist Hymnal"
  ["LSB2006"]="Lutheran Service Book"
  ["BH1991"]="Baptist Hymnal"
  ["AAHH2001"]="African American Heritage Hymnal"
  ["CBoW1994"]="Common Book of Worship"
)

for code in "${!hymnals[@]}"; do
  name="${hymnals[$code]}"
  filename="scripts/import-${code,,}.ts"
  
  cat > "$filename" << EOF
/**
 * Import hymns from ${name} (${code}) on Hymnary.org
 *
 * Usage: npx tsx scripts/import-${code,,}.ts
 */

import { prisma } from '../lib/db/prisma';
import { parseHymnText } from '../lib/hymn-processor/parser';

const HYMNAL_CODE = '${code}';
...
async function main() {
  console.log(\`Starting import from ${name} (\${HYMNAL_CODE})...\`);
...
echo "All import scripts created!"
